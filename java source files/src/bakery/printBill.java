...
public class printBill extends javax.swing.JFrame {
        ...
        if((cbo_bNumber.getSelectedItem().toString()).equals("")) {    //Check to see if bill details has been selected
            ...
            if (dialogResult==JFileChooser.APPROVE_OPTION){
                ...
                try {   
                    ...
                    myDocument.add(new Paragraph("BAKERY",FontFactory.getFont(FontFactory.TIMES_BOLD,25,Font.BOLD )));
    ...
    private void cbo_bNumberItemStateChanged(java.awt.event.ItemEvent evt) {//GEN-FIRST:event_cbo_bNumberItemStateChanged
        ...
        if (listLoaded) {
            String sql = "SELECT bDate, bCustName, SUM(bAmount) FROM billing WHERE bNumber=? AND bOk=? GROUP BY bDate, bCustName";
            ...
            new SwingWorker<Void, Void>() {
                ...
                protected Void doInBackground() throws Exception {
                    try (PreparedStatement pst = conn.prepareStatement(sql)) {
                        ...
                        try (ResultSet rs = pst.executeQuery()) {
                            if (rs.next()) {  // Move result set to the first row
                                billDate = rs.getDate("bDate");
                                customerName = rs.getString("bCustName");
                                billAmount = rs.getString(3); // Assuming SUM(bAmount) is the 3rd column
                            } else {
                                JOptionPane.showMessageDialog(null, "No billing details found for the selected number.");
                            }
                        }
                ...
                @Override
                protected void done() {
                    // Update the GUI components with the retrieved values
                    if (billDate != null) {
                        txt_bDate.setDate(billDate);
                    }
                    txt_bCustName.setText(customerName != null ? customerName : "");
                    txt_bAmount.setText(billAmount != null ? billAmount : "0");
                }
            }.execute();
        }
